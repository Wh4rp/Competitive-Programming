...
// lps[i] = length of the longest proper prefix which is also suffix in
// pattern[0 .. i]
void init_lps(string& pattern, int lps[]) {
  int n = pattern.size();
  lps[0] =
      0;  // base case: no proper prefix/suffix for pattern[0 .. 0] (length 1)
  rep(j, 1, n) {  // for each pattern[0 .. j]
    int i =
        lps[j - 1];  // i points to the char next to lps of previous iteration
...
// Count number of matches of pattern string in target string using KMP
// algorithm
int count_matches(string& pattern, string& target) {
  ...
  rep(j, 0, m) {  // j tracks each char in target to compare
    // try to keep prefix before i as long as possible while ensuring i matches
    // j
    while (pattern[i] != target[j] and i > 0)
      i = lps[i - 1];
    if (pattern[i] == target[j] || pattern[i] == '?' || target[j] == '?') {
